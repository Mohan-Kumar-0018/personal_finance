...
def mark_cash_transfers(banks_df):
	# Find matching rows with the same non-empty TRANSACTION_ID
	cash_transactions = banks_df[(banks_df['ACCOUNT_TYPE'] == 'BANK') & (banks_df['ACCOUNT'].str.upper().str.contains('CASH'))]
	...
	for index, row in cash_transactions.iterrows():
		...
		if matching_row is not None:
			...
			banks_df.at[matching_row, 'REMARKS'] = f"{row['S_NO']} - {row['ACCOUNT']}"
			...
			logging.info(f"Marked transfer: Bank transaction at index {banks_df.at[matching_row, 'S_NO']} matches Cash transaction {row['S_NO']}")
		else:
			logging.debug(f"No matching bank transaction found for Cash transaction {row['S_NO']}")
