...
def mark_splitwise_transfers(banks_df, splitwise_transaction_df):
	for index, row in splitwise_transaction_df.iterrows():
		...
		if matching_row is not None:
			...
			print(f"Marked transfer: Bank transaction at index {banks_df.at[matching_row, 'S_NO']} matches Splitwise transaction {row['S_NO']}")
		else:
...
def mark_pnb_transfers(banks_df):
	...
	for index, row in pnb_transactions.iterrows():
		...
		if matching_row is not None:
			...
			print(f"Marked transfer: Bank transaction at index {banks_df.at[matching_row, 'S_NO']} matches PNB transaction {row['S_NO']}")
		else:
			print(f"No matching bank transaction found for PNB transaction {row['S_NO']}")
	return banks_df
